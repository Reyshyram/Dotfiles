...
clip_h=$(cat "$HOME/.config/rofi/emoji.rasi" | awk '/window {/,/}/'  | awk '/height:/ {print $2}' | awk -F "%" '{print $1}')
clip_w=$(cat "$HOME/.config/rofi/emoji.rasi" | awk '/window {/,/}/'  | awk '/width:/ {print $2}' | awk -F "%" '{print $1}')
...
# clipboard action
sed '1,/^### DATA ###$/d' $0 | rofi -dmenu -theme-str "entry { placeholder: \"Copy...\";} ${pos} ${r_override}" -config ~/.config/rofi/emoji.rasi | cut -d ' ' -f 1 | tr -d '\n' | wl-copy
